...
private[cp14_表明と単体テスト] abstract class Element { //cp10から転用
    ...
    private def widen(that: Element): Element =
        if ((this.width max that.width) == this.width)
            ...
            elem(this.contents.map { s =>
                ...
                left + s + right
            }) ensuring ((this.width max that.width) == _.width) //ここで条件が真ならば結果値が返され、偽ならば例外が投げられる。
...
private[cp14_表明と単体テスト] object Element { //クラスElementのコンパニオンオブジェクト
